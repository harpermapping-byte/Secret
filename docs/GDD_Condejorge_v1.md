...
3. **Tregua** — durante la ronda **siguiente** a desbloquearse, ningún ataque de ninguna otra facción le hace nada a esta: es como si tuviera una alianza automática con todo el mundo esa ronda, sin que nadie tenga que votarla ni depender de que las alianzas estén activadas en la partida.
4. **Castillo especial** — aparece un castillo junto a la capital de la facción (placeholder teñido de su color, igual mecanismo que la capital misma), que trae **2 tropas especiales** al construirse y produce **1 más cada ronda** después, hasta un tope de **10 por facción** (al llegar al tope deja de producir). Cada tropa especial aporta **0.4 fijo tanto de ataque como de defensa** a su facción, siempre, sin que nadie tenga que votar nada por ellas — a diferencia de las tropas de IA normales, no van "detrás" de ningún jugador ni dependen de que alguien vote `!ataque`/`!defender`. Sin aldeanos paseando alrededor, a diferencia de la capital.
...
Cada torre terminada da **+0.5 de defensa pasiva** a su facción, para siempre — una de las dos únicas excepciones a "el territorio no se defiende solo" (la otra son las tropas especiales del castillo del nivel 4 de industria, ver más abajo; ver Combate para las dos juntas): el bonus de las torres se suma SIEMPRE, aunque nadie escriba `!defender` esa ronda, y se acumula con la defensa que sí aporte la gente cuando la haya. Si el terreno donde está la torre cambia de dueño (terminada o a medias), la torre se la lleva quien lo conquiste, igual que cualquier otro edificio.
...
**El territorio no se defiende solo.** Una facción a la que nadie defiende esa ronda entra al combate con **0 de defensa** — más el bonus fijo de sus torres terminadas si tiene alguna (ver `!torre` más arriba), más el de sus tropas especiales del castillo del nivel 4 de industria si lo tiene, más el de sus maravillas de defensa si posee alguna (ver Maravillas más arriba) — estas tres son las ÚNICAS excepciones, por muchas casillas que tenga la facción: el resto de la defensa sale de los `!defender` de esa ronda. Las tropas especiales, además, hacen lo mismo también **atacando**: si tu facción tiene alguna, se suman al ataque de tu facción en cuanto alguien de ella vote `!ataque` esa ronda (no hace falta que las tropas especiales "voten" nada, es un bonus fijo por tenerlas) — pero, como siempre, sin nadie atacando no hay combate que resolver.
