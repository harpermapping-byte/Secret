...
| `!casas` | Fase de Acción | Construye una vivienda junto a tu capital (un voto = una vivienda, tope 10 por facción): +5 al límite de tropas de CADA jugador de tu facción por vivienda en pie. |
| `!apoyar <usuario>` | Fase de Acción | P.ej. `!apoyar Rulo`. Mandas tus tropas (y a tu jugador, que viaja a la capital elegida) a reforzar la DEFENSA de otra facción esa ronda — un préstamo puntual de un turno, no cambias de bando. Solo cuenta ese turno: si esa facción es atacada, tus tropas suman a su defensa y corren el mismo riesgo que las suyas (ver "Sistema de vidas" en Eliminación); si no la atacan, no pasa nada. Inválido con tu propia facción, con un usuario inexistente, o con uno ya muerto. |
...
### Viviendas: `!casas`
Disponible desde el principio, sin toggle de admin. Cada `!casas` votado levanta una vivienda junto a la capital de tu facción (mismo anillo anti-solape que estatua/museo/iglesia), un voto = una vivienda, hasta un máximo de **10 por facción**. No da tropas: sube **+5 permanente** el límite de tropas que puede llevar CADA jugador de tu facción, por cada vivienda que tenga la facción — hasta +50 en total, acumulable con el +50 de la iglesia (nivel 3 de industria) sin tope combinado entre las dos.
