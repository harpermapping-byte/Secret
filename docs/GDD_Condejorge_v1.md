...
| `!alianza <nº facción>` | Fase de Acción, solo si el admin activó alianzas en esta partida | Propone una alianza a esa facción. Si en la MISMA ronda las dos facciones se lo proponen mutuamente (cada una superando su % de votos), se forma una alianza de varias rondas (duración configurable por el admin, 5 por defecto): no pueden atacarse entre sí y defienden como una sola facción. |
...
| `!casas` | Fase de Acción | Construye una vivienda junto a tu capital (un voto = una vivienda, tope 10 por facción): +10 al límite de tropas de CADA jugador de tu facción por vivienda en pie. |
| `!apoyar <nº facción>` | Fase de Acción | P.ej. `!apoyar 2`. Mandas tus tropas a reforzar la DEFENSA de esa facción esa ronda — un préstamo puntual de un turno, no cambias de bando. Solo cuenta ese turno: si esa facción es atacada, tus tropas suman a su defensa y corren el mismo riesgo que las suyas (ver "Sistema de vidas" en Eliminación); si no la atacan, no pasa nada. Inválido con tu propia facción o con una facción inexistente/eliminada. |
...
- **Fase de Resolución** (se repite cada ronda, sustituye al antiguo paron de transición justo tras la Fase de Acción) — la ronda ya está resuelta en el servidor de golpe, pero se REPRODUCE de cara al espectador combate a combate, en orden: la cámara se acerca (zoom cercano, no al máximo) a cada combate/asalto PvE/conquista de la ronda, con una polvareda entre los dos bandos si es un combate (PvP o PvE) o un carromato con lona viajando hacia la casilla si es una conquista, mientras un cartel no bloqueante cuenta el detalle (daño, defensa, tropas perdidas, si murió alguien) — el mapa se sigue viendo detrás, no es un popup a pantalla completa. La duración total de la fase se adapta sola: si hay pocos eventos se reproducen a ritmo normal, si hay muchos (muchas facciones, muchos combates a la vez) se acortan proporcionalmente para no comerse el stream entero. El admin puede pasar de ronda a mitad de esta fase (repasa cada evento restante en modo rápido, tarjeta a tarjeta, sin cámara ni efectos — nunca salta directo al resumen). Ver `docs/ACCIONES.md` sección 34.
...
3. **Iglesia** — aparece una iglesia junto a la capital de la facción (placeholder propio, igual mecanismo de anillo que el trofeo de dungeon/boss) que da **+25 permanente** al límite de tropas de cada jugador de la facción, mientras dure la partida. Se acumula con el bono de las viviendas de `!casas` (ver más abajo) sin tope combinado entre las dos.
...
Votando `!dungeon` atacas, junto con el resto de tu facción, a un dungeon que esté dentro de tu propio territorio — comando aparte de `!conquista` porque la recompensa es distinta. Mismo combate bidireccional que `!conquista`: tu ataque decide si lo derrotas, el ataque de la guarnición contra tu defensa os causa bajas siempre, ganéis o no — igual que con `!conquista`, aquí sí se puede morir, y una muerte deja la guarnición debilitada de forma permanente para el siguiente intento. Al derrotarlo, la guarnición desaparece para siempre y tu facción gana una **estatua** nueva junto a su capital (no producción para la casilla) — con sus propios 4 aldeanos paseando alrededor, igual que la capital. Varias estatuas se reparten en anillo alrededor de la capital sin amontonarse, así que la capital "va creciendo" visiblemente según se derrotan más dungeons.

### Viviendas: `!casas`
Disponible desde el principio, sin toggle de admin. Cada `!casas` votado levanta una vivienda junto a la capital de tu facción (mismo anillo anti-solape que estatua/museo/iglesia), un voto = una vivienda, hasta un máximo de **10 por facción**. No da tropas: sube **+10 permanente** el límite de tropas que puede llevar CADA jugador de tu facción, por cada vivienda que tenga la facción — hasta +100 en total, acumulable con el +25 de la iglesia (nivel 3 de industria) sin tope combinado entre las dos.
...
### Alianzas (por pacto mutuo, de varias rondas)
Configurable sí/no en la Fase 0. Si están activas, una alianza se forma por PACTO MUTUO: hace falta que las DOS facciones se voten `!alianza` la una a la otra en la MISMA Fase de Acción, cada una alcanzando el % configurado (por defecto 50%) de sus votantes activos. La alianza dura las rondas que configure el admin (por defecto 5) y mientras esté activa: (1) los `!ataque` entre las dos aliadas quedan anulados (el votante pierde el turno); (2) defienden como UNA SOLA facción — si atacan a una, los `!defender` de la otra cuentan también en ese combate. Al agotarse las rondas vuelven a ser independientes (y pueden volver a pactar). Dos casos especiales:
- Si un voto de alianza **no llega** al % necesario dentro de su facción, todos los que votaron `!alianza` ese turno cuentan como neutrales/inactivos esa ronda (no defienden, no atacan, no producen).
- Si una facción supera su % pero la otra NO le corresponde esa misma ronda, no pasa nada: la oferta caduca sin efecto y los votantes no pierden el turno.
...
### Sistema de vidas
Cada jugador empieza la partida con `startingLives` vidas (configurable por el admin, 1-5, por defecto 3 — 1 vida es muerte súbita, como el comportamiento clásico). Quedarse sin ninguna tropa en un combate (PvP **o** PvE — antes solo PvE mataba de verdad, ahora los dos comparten la misma regla) cuesta una vida: si todavía le queda alguna, **reaparece** con 0 tropas en su territorio (sigue vivo, sigue jugando); si era su última vida, ahí sí es la muerte real de siempre (puede arrastrar la eliminación de su facción si era el último jugador vivo). Las vidas restantes se ven como corazones (❤️ = con ella, 🖤 = ya perdida) junto al nombre de cada jugador: en el panel de Facciones, en el panel de Jugadores, y encima de su propio personaje en el mapa.
...
6. Al cerrar la Fase de Acción, ve la Fase de Resolución: la cámara va acercándose a cada combate/asalto/conquista de la ronda, uno a uno, con su polvareda o carromato y un cartel contando el detalle. Después, la Fase de Resumen muestra en bloques (10-15s cada uno) qué pasó: ataques, industria, bajas, conquistas, especiales, mejoras desbloqueadas.
7. El ciclo Acción → Resolución → Resumen se repite ronda tras ronda hasta que su facción gana, es eliminada, o él mismo pierde su última vida en combate (antes de eso, quedarse sin tropas en un combate solo le hace perder una vida y reaparecer con 0 tropas, ver "Sistema de vidas" en la sección 6).
