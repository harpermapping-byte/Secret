...
**Pendiente de esta misma petición, para el siguiente lote** (el pedido original venía con mucho más — se separó en lotes para poder verificar cada pieza, en vez de un cambio enorme sin probar): generación de castillo/aldea/puerto por probabilidad-por-casilla (reemplazando el conteo fijo actual), guarnición/aldeanos visibles paseando alrededor del edificio, el nuevo modelo de combate de tropas por absorción de daño en cascada (caballero→arquero→leva) tanto contra bandidos como entre jugadores, movimiento en cono desincronizado para las tropas, quitar el tinte semitransparente del sprite del jugador y poner un puntito de color de facción junto al nombre, agrandar los placeholders de barraca/campo de arquería/caballeriza, radio anti-solape entre elementos del mapa, capitales de facción con aldeanos, y recortar la lista de comandos a solo nombre + explicación breve (la explicación larga se queda en el modal de ayuda).
