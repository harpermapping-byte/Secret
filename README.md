...
- **Mapa estilo Risk, como un fondo real**: el mapa es un rectángulo a pantalla completa dividido en tantos territorios irregulares como casillas haya puesto el admin (nada de cuadrícula uniforme) — fronteras dibujadas tipo Voronoi, con zoom (rueda del ratón o botones **+ / −**) y arrastre para moverte. Se comporta como un mapa de fondo tipo Google Maps: el zoom-out máximo siempre deja el mapa ocupando toda la pantalla (nunca más pequeño ni con hueco vacío alrededor) y no se puede arrastrar más allá de sus bordes. Es la misma adyacencia que usa el motor de verdad: si dos territorios se tocan en el mapa, son vecinos para ataques/expansión. Sigue siendo placeholder de color, sin arte final — ver `server/mapTemplates.js` y `public/mapRenderer.js` (el módulo que dibuja el mapa, compartido entre la web pública y el panel de admin, para que ambos lo vean igual).
