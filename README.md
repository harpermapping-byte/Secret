...
- **Mapa mundial real, estilo Risk**: el mapa es el planeta real (silueta de continentes y océanos, sin la Antártida — esa zona no tiene ciudades ni combate) con un margen arriba y abajo (no llega pegado al borde de la pantalla, como en streamer-wars.com), dividido en tantos territorios irregulares como casillas haya puesto el admin — pero solo la tierra se reparte, el océano se queda como fondo, sin dueño y sin poder conquistarse. Las piezas no respetan fronteras de países reales, son cortes irregulares tipo Risk. Tiene zoom (rueda del ratón o botones **+ / −**) y arrastre para moverte. Se comporta como un mapa de fondo tipo Google Maps: el zoom-out máximo siempre deja el mapa ocupando todo su espacio (nunca más pequeño ni con hueco vacío alrededor) y no se puede arrastrar más allá de sus bordes — como el océano ya ocupa la mayor parte del lienzo, eso da aire de sobra alrededor de la tierra sin verse "ahogado". El zoom máximo es moderado, pensado para ver bien las animaciones de combate, no para acercarse al detalle. La resolución del raster se subió dos veces (10x en total sobre la primera versión) para que las costas y fronteras no se vean "a bloques" ni siquiera con el zoom máximo. Es la misma adyacencia que usa el motor de verdad: si dos territorios de tierra se tocan en el mapa, son vecinos para ataques/expansión. La silueta del planeta sale de datos públicos de costas (dominio público, sin copyright) — sigue siendo un placeholder de color plano, sin arte final — ver `server/worldLandMask.js` (la silueta), `server/mapTemplates.js` (el reparto en territorios) y `public/mapRenderer.js` (el módulo que dibuja el mapa, compartido entre la web pública y el panel de admin, para que ambos lo vean igual).
- **Marcadores de jugador en el mapa**: en cuanto alguien se une a una facción (`!faccion1`), aparece un triángulo del color de su facción sobre el territorio de esa facción, con su nombre pequeño encima (se lee bien al hacer zoom, no satura la vista general). Si el jugador muere, su marcador desaparece. Es un esqueleto a propósito: la posición no se mueve todavía según lo que haga el jugador (eso — animarlo al atacar, defender, hacer industria o morir — es trabajo futuro); hoy solo marca "aquí está, vivo, en esta facción".
- **Web pública**: arriba a la izquierda, un botón **🏳️ Facciones** (mismo mecanismo que "👥 Jugadores", pero desplegándose por la izquierda) sustituye a los chips que antes estaban siempre visibles en pantalla — sale ya desplegado la primera vez que empieza la partida, y a partir de ahí se pliega/despliega con el botón, para mantener la pantalla más limpia. Dentro del panel **👥 Jugadores** hay ahora una caja de búsqueda: escribe el nombre de un jugador y el mapa hace zoom hasta su marcador. Al entrar en la Fase de Resumen se abre automáticamente un popup con lo que pasó esa ronda, separado en Conquistas / Industria / Combates / Bajas / Eliminaciones. También hay un botón **🏆 Clasificación** arriba que abre un popup con la tabla de facciones (soldados, tanques, territorios, maravillas — reservado para más adelante —, industria de la última ronda, bajas causadas), y un botón **❓ Comandos** que abre un popup con la lista de comandos de chat (qué escribir, en qué fase, y qué hace cada uno) — útil también para entender por qué un comando "no hizo nada" si se escribió en la fase equivocada. Al terminar la partida, el mapa y las estadísticas se quedan visibles tal cual (nada se limpia); un aviso flotante arriba avisa de que terminó, y automáticamente (una vez por partida) se abre un resumen final a pantalla completa con las tarjetas de cada facción (vivos/muertos, territorios finales, bajas causadas) — se puede cerrar y volver a abrir con el enlace "ver resumen" del aviso, hasta que el admin cree una partida nueva.
- **Eliminación de facciones**: si una facción se queda sin ningún jugador vivo (por combate normal, Bombardeo u Operación especial), su territorio no se queda plantado esperando conquista casilla a casilla — pasa a neutral de golpe (menos la casilla que ya gana quien dio el golpe final, si la hubo, siempre en su propia frontera). No es como en Risk: quien la elimina no hereda todo su territorio, solo esa casilla; el resto queda libre para que cualquier otra facción lo tome después. La partida en sí solo termina cuando ya únicamente queda una facción con territorio — con 3 o más facciones en juego, eliminar a una no acaba la partida si las demás siguen vivas.
- **Panel de admin**: mismo mapa a pantalla completa que la web pública, con un panel lateral (botón **📋 Facciones**) con tarjetas por facción (industria, territorios, bajas causadas, jugadores), una sección **Bot de Twitch** que muestra si el bot está realmente conectado y unido al canal de chat configurado (con el último error si lo hay), un registro de los últimos comandos de chat reconocidos (aceptados o rechazados, con el motivo — por ejemplo "fase incorrecta"), y el JSON en crudo plegado por si hace falta para depurar más a fondo. Esta sección de diagnóstico existe porque, si el bot no llega a conectar o el streamer/jugador escribe el comando en la fase equivocada, antes no había forma de saberlo desde la web — ahora se ve de un vistazo. Al terminar la partida, los botones de "en curso" (pausar/pasar ronda/terminar) se cambian por uno de **🔄 Nueva partida**, que vuelve a mostrar el formulario de configuración para crear otra con los ajustes que quieras — sin tener que reiniciar el servidor ni la página.
- **Pausar/reanudar de verdad muestra la cuenta atrás**: tanto el panel de admin como la web pública muestran ahora un timer con la cuenta atrás de la fase en curso; al pulsar **Pausar**, se congela con un icono ⏸ en vez de seguir bajando hasta 00:00 (antes parecía que "pausar no funcionaba" porque el número seguía cayendo aunque el servidor ya había parado el reloj de verdad), y el botón de Pausar se deshabilita mientras está en pausa (igual que Reanudar se deshabilita si no lo está). Al pulsar **Reanudar**, retoma la cuenta atrás exactamente donde se quedó.
...
- La resolución del raster (4400×2302 celdas, recortado para excluir la Antártida) está pensada para que las costas y fronteras se vean nítidas incluso con el zoom máximo del juego, y para dejar sitio a detalle futuro (ciudades, etc.); generar el mapa tarda algo más que antes al subir la resolución (~0.6-1.1s medido, según cuántos territorios pida el admin) pero sigue siendo una sola vez por partida (al pulsar "Crear partida"), no en cada frame, así que no afecta a la fluidez del juego ya en marcha. Si en el futuro hiciera falta aún más resolución y ese tiempo empezara a notarse, el siguiente paso sería optimizar la búsqueda de territorio más cercano en `server/mapTemplates.js` con una estructura espacial en vez de seguir subiendo solo la resolución.
...
- **Diagnóstico del bot de Twitch**: si el chat no reacciona a comandos como `!faccion1`, el panel de admin ahora muestra el estado real de la conexión del bot (conectando/conectado/error/reconectando, a qué canales se unió de verdad, último error, último mensaje de chat visto) y un registro de los últimos comandos de chat reconocidos con el motivo de si se aceptaron o no — la causa más común de "escribí el comando y no pasó nada" es que la partida no está en la fase que ese comando necesita (por ejemplo, `!faccion1` solo funciona en la Fase de Reclutamiento, después de pulsar "Iniciar partida"), y eso ahora se ve directamente ahí en vez de tener que adivinarlo.
- **Rendimiento del mapa**: el repintado del mapa está optimizado para no bloquear la página — la clasificación de cada celda (borde/costa/océano) se calcula una sola vez por partida en vez de en cada repintado, y un repintado que no cambia ningún territorio (por ejemplo, uno disparado solo por el estado del bot de Twitch) no vuelve a recorrer el mapa entero. Esto arregló una regresión real: justo después de subir la resolución del mapa, el botón "Iniciar partida" podía sentirse "colgado" durante varios segundos porque el navegador se quedaba ocupado repintando el mapa completo repetidas veces. Ver `docs/ACCIONES.md` sección 6 para el detalle técnico.
- **Peso del mensaje del mapa**: la geometría del mapa (`cellTileIds`, un id de territorio por cada una de las ~10 millones de celdas del raster) se manda empaquetada en 1-2 bytes/celda + base64 (`server/mapLayoutCodec.js`) en vez de como un array JSON plano — el array plano pesaba ~27,5MB por mensaje; empaquetado baja a ~13MB, y sobre todo cambia cómo lo procesa el navegador: en vez de parsear ~10 millones de números sueltos, parsea un único string largo, mucho más rápido. Sigue habiendo margen de mejora (mandar el mapa como una imagen ya renderizada en el servidor, en vez de datos crudos, comprimiría bastante más) — está en discusión, no implementado todavía.
